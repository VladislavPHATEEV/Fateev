...
class DataSet:
    ...
    def get_stats(self):
        ...
        stats2 = self.avg(salary_of_vacancy_name)
...
class Report:
    ...
    def gen_excel(self):
        ws11 = self.wb.active
        ws11.title = 'Статистика по годам'
        ws11.append(['Год', 'Средняя зарплата', 'Средняя зарплата - ' + self.vacancy_name, 'Количество вакансий', 'Количество вакансий - ' + self.vacancy_name])
        for year in self.stats1.keys():
            ws11.append([year, self.stats1[year], self.stats3[year], self.stats2[year], self.stats4[year]])
        ...
        for i, column_width in enumerate(column_widths, 1):  # ,1 to start at 1
            ws11.column_dimensions[get_column_letter(i)].width = column_width + 2
        data = []
        ...
        ws22 = self.wb.create_sheet('Статистика по городам')
        for row in data:
            ws22.append(row)
        column_widths = []
        for row in data:
            for i, cell in enumerate(row):
                ...
                if len(column_widths) > i:
                    ...
                    column_widths += [len(cell)]
        for i, column_width in enumerate(column_widths, 1):  # ,1 to start at 1
            ws22.column_dimensions[get_column_letter(i)].width = column_width + 2
        font_bold = Font(bold=True)
        for col in 'ABCDE':
            ws11[col + '1'].font = font_bold
            ws22[col + '1'].font = font_bold
        for index, _ in enumerate(self.stats5):
            ws22['E' + str(index + 2)].number_format = '0.00%'
        thin = Side(border_style='thin', color='00000000')
        for row in range(len(data)):
            for col in 'ABDE':
                ws22[col + str(row + 1)].border = Border(left=thin, bottom=thin, right=thin, top=thin)
        for row, _ in enumerate(self.stats1):
            for col in 'ABCDE':
                ws11[col + str(row + 1)].border = Border(left=thin, bottom=thin, right=thin, top=thin)
